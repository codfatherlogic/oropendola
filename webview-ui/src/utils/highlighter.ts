...
const initialLanguages: BundledLanguage[] = ["shell", "log", "javascript", "typescript", "python", "json"]
	...
	try {
		...
		if (!state.instanceInitPromise) {
			state.instanceInitPromise = (async () => {
				console.log("[Shiki] Initialization started...")
				...
				console.log("[Shiki] Initialization complete")
			...
			if (!loadingPromise) {
				...
				loadingPromise = (async () => {
					try {
						...
						console.log(`[Shiki] Loaded language ${shikilang}`)
					} catch (error) {
