...
`;
    ...
    _setupTaskEventListeners(task) {
        ...
        // Conversation saved
        task.on('conversationSaved', (filepath) => {
...
<body>
    ...
    _getChatHtml(webview) {
        ...
        '<meta name="webview-version" content="' + cacheBuster + '">' +
        '<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.9.0/styles/vs2015.min.css">' +
        '<script src="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.9.0/highlight.min.js"></script>' +
        '<title>Oropendola AI Chat</title>' +
        '<style>' +
        '* { margin: 0; padding: 0; box-sizing: border-box; }' +
        'body { font-family: var(--vscode-editor-font-family), "SF Mono", Monaco, "Cascadia Code", Consolas, "Courier New", monospace; font-weight: 400; font-size: 13px; color: var(--vscode-foreground); background-color: var(--vscode-sideBar-background); height: 100vh; display: flex; flex-direction: column; -webkit-font-smoothing: subpixel-antialiased; -moz-osx-font-smoothing: auto; text-rendering: geometricPrecision; }' +
        ...
        '.messages-container { flex: 1; overflow-y: auto; padding: 16px 18px; display: flex; flex-direction: column; gap: 16px; }' +
        '.message { padding: 12px 16px 12px 28px; border-radius: 0; word-wrap: break-word; font-family: inherit; font-size: 13px; font-weight: 400; line-height: 1.6; position: relative; border: none; display: flex; flex-direction: column; min-height: 40px; background: transparent; }' +
        '.message-user { border: none; border-radius: 0; position: relative; overflow: visible; padding-left: 28px; background: transparent; }' +
        '.message-user::before { content: ""; position: absolute; left: 8px; top: 8px; width: 8px; height: 8px; background: #40a5ff; border-radius: 50%; }' +
        '.message-assistant { border: none; border-radius: 0; position: relative; overflow: visible; padding-left: 28px; background: transparent; }' +
        '.message-assistant::before { content: ""; position: absolute; left: 8px; top: 8px; width: 8px; height: 8px; background: #2ecc71; border-radius: 50%; }' +
        '.message-error { border: none; border-radius: 0; position: relative; overflow: visible; padding-left: 28px; background: transparent; }' +
        '.message-error::before { content: ""; position: absolute; left: 8px; top: 8px; width: 8px; height: 8px; background: #e74c3c; border-radius: 50%; }' +
        '.message-system { background-color: transparent; color: var(--vscode-descriptionForeground); font-style: italic; padding: 12px 16px 12px 28px; border: none; }' +

        '.message-content { padding-left: 0; min-width: 0; overflow-wrap: break-word; white-space: pre-wrap; color: var(--vscode-editor-foreground); display: block !important; min-height: 20px; font-family: inherit; font-size: 13px; font-weight: 400; line-height: 1.6; letter-spacing: normal; text-rendering: geometricPrecision; }' +
        '.message-content strong { font-weight: 500; color: var(--vscode-foreground); }' +
        '.message-content br { line-height: 1.6; }' +
        '.message pre { background: var(--vscode-textCodeBlock-background); padding: 16px 18px; border-radius: 8px; overflow-x: auto; margin: 16px 0; border: 1px solid rgba(255, 255, 255, 0.06); font-family: inherit; }' +
        '.message code { font-family: inherit; font-size: 12px; font-weight: 400; background: var(--vscode-textCodeBlock-background); padding: 2px 6px; border-radius: 3px; }' +
        '.message pre code { background: none; padding: 0; font-family: inherit; font-size: 12px; font-weight: 400; line-height: 1.5; letter-spacing: normal; }' +
        '.message-actions { display: flex !important; flex-direction: row; gap: 10px; margin-top: 16px; padding-top: 14px; border-top: 1px solid rgba(255, 255, 255, 0.08); justify-content: flex-end; }' +
        ...
        '.code-block-enhanced { position: relative; margin: 16px 0; border-radius: 8px; background: var(--vscode-textCodeBlock-background); border: 1px solid rgba(255, 255, 255, 0.06); overflow: hidden; }' +
        '.code-header { display: flex; justify-content: space-between; align-items: center; padding: 10px 16px; background: rgba(0, 0, 0, 0.15); border-bottom: 1px solid rgba(255, 255, 255, 0.06); }' +
        '.code-language { font-size: 10.5px; color: var(--vscode-descriptionForeground); text-transform: uppercase; font-weight: 600; letter-spacing: 0.8px; opacity: 0.8; }' +
        ...
        '.code-block-enhanced pre { margin: 0; padding: 16px 18px; overflow-x: auto; background: transparent; }' +
        '.code-block-enhanced code { font-family: "Fira Code", "JetBrains Mono", var(--vscode-editor-font-family), "SF Mono", Monaco, Consolas, monospace; font-size: 13px; font-weight: 400; line-height: 1.5; letter-spacing: 0; }' +
        '.file-link { color: inherit; text-decoration: none; border: none; cursor: pointer; transition: none; font-weight: normal; }' +
        '.file-link:hover { color: inherit; border: none; background: none; padding: 0; }' +
        '.typing-indicator { padding: 12px 16px 12px 28px; margin-top: 0; border-radius: 0; font-family: inherit; font-size: 13px; font-weight: 400; line-height: 1.6; position: relative; border: none; display: flex; align-items: center; gap: 10px; background: rgba(255, 193, 7, 0.08); color: var(--vscode-foreground); min-height: 44px; opacity: 1; border-left: 3px solid #FFC107; }' +
        '.typing-indicator::before { content: ""; position: absolute; left: 8px; top: 50%; transform: translateY(-50%); width: 8px; height: 8px; background: #FFC107; border-radius: 50%; animation: pulse 1.5s ease-in-out infinite; }' +
        '@keyframes pulse { 0%, 100% { opacity: 1; transform: translateY(-50%) scale(1); } 50% { opacity: 0.5; transform: translateY(-50%) scale(0.8); } }' +
        '.typing-text { font-size: 13px; font-weight: 400; color: var(--vscode-foreground); letter-spacing: normal; }' +
        ...
        '.ai-progress-message { display: flex; align-items: flex-start; gap: 14px; padding: 14px 18px; margin: 10px 0; border-radius: 8px; animation: slideIn 0.3s ease; align-self: flex-start; max-width: 90%; }' +
        ...
        '.todo-panel { background: var(--vscode-editor-background); border: 1px solid var(--vscode-panel-border); border-radius: 10px; margin: 16px 18px; padding: 14px; display: none; }' +
        '.todo-panel.visible { display: block; }' +
        '.todo-panel.collapsed .todo-content { display: none; }' +
        '.todo-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 14px; padding-bottom: 10px; border-bottom: 1px solid var(--vscode-panel-border); cursor: pointer; user-select: none; }' +
        ...
        '.todo-item { display: flex; align-items: center; gap: 12px; padding: 10px 12px; border-radius: 6px; font-size: 13px; transition: background 0.2s; cursor: pointer; line-height: 1.6; }' +
        ...
        '.copilot-changes-container { background: transparent; border: none; border-radius: 0; margin: 12px 0; overflow: visible; font-family: var(--vscode-font-family); }' +
        '.copilot-changes-header { padding: 8px 0; background: transparent; border-bottom: none; }' +
        '.copilot-changes-count { font-size: 12px; font-weight: 600; color: var(--vscode-foreground); }' +
        '.copilot-changes-list { padding: 0; }' +
        '.copilot-change-item { margin: 4px 0; background: transparent; border-radius: 0; padding: 4px 0; transition: all 0.2s; }' +
        '.copilot-change-item.undoing { transition: all 0.3s ease-out; }' +
        '.copilot-change-item.kept { opacity: 0.6; }' +
        '.copilot-change-item:hover { background: transparent; }' +
        ...
        '</div>' +
        '<script>' +
        'console.log("[WEBVIEW] Script starting - version: ' + cacheBuster + '");' +
        'try {' +
        'const vscode = acquireVsCodeApi();' +
        ...
        'console.log("[WEBVIEW] All DOM elements found:", {messagesContainer: !!messagesContainer, messageInput: !!messageInput});' +
        'if (!messagesContainer) { document.body.innerHTML = "<div style=\\"padding: 20px; color: red;\\">ERROR: messagesContainer not found!</div>"; throw new Error("messagesContainer not found"); }' +
        'console.log("[WEBVIEW] Chat interface loaded successfully - version: ' + cacheBuster + '");' +
        ...
        'const thinkingWords = ["Thinking", "Crafting", "Elucidating", "Churning", "Working", "Forging", "Creating", "Scheming", "Schlepping", "Cogitating", "Ruminating", "Conceiving", "Designing", "Building", "Formulating", "Inventing", "Shaping", "Composing", "Constructing", "Fabricating", "Strategizing", "Plotting", "Calculating", "Conceptualizing", "Envisioning", "Articulating", "Illuminating", "Clarifying", "Expounding", "Interpreting", "Refining", "Iterating", "Evolving", "Reworking", "Polishing", "Reimagining", "Optimizing", "Manifesting", "Synthesizing", "Ideating", "Curating", "Transmuting", "Infusing", "Galvanizing", "Incubating", "Honing", "Dreaming", "Pursuing", "Innovating", "Engineering", "Developing", "Executing", "Producing", "Mobilizing", "Driving", "Operating", "Delivering", "Grinding", "Hustling", "Believing", "Inspiring", "Fueling", "Kindling", "Empowering", "Transforming", "Imagining", "Sculpting", "Assembling", "Drafting", "Rendering", "Molding", "Finishing"];' +
        'function showTypingIndicator() { if (emptyState) emptyState.style.display = "none"; if (typingElement && typingElement.parentNode) { return; } hideTypingIndicator(); typingElement = document.createElement("div"); typingElement.className = "typing-indicator"; const textSpan = document.createElement("span"); textSpan.className = "typing-text"; let wordIndex = 0; textSpan.textContent = thinkingWords[wordIndex]; typingElement.appendChild(textSpan); const dotsContainer = document.createElement("div"); dotsContainer.className = "typing-dots"; for (let i = 0; i < 3; i++) { const dot = document.createElement("div"); dot.className = "typing-dot"; dotsContainer.appendChild(dot); } typingElement.appendChild(dotsContainer); messagesContainer.appendChild(typingElement); messagesContainer.scrollTop = messagesContainer.scrollHeight; if (typingRotationInterval) clearInterval(typingRotationInterval); typingRotationInterval = setInterval(function() { if (textSpan && typingElement && typingElement.parentNode) { wordIndex = (wordIndex + 1) % thinkingWords.length; textSpan.textContent = thinkingWords[wordIndex]; } else { clearInterval(typingRotationInterval); typingRotationInterval = null; } }, 2000); }' +
        'function hideTypingIndicator() { if (typingRotationInterval) { clearInterval(typingRotationInterval); typingRotationInterval = null; } if (typingElement) { typingElement.remove(); typingElement = null; } }' +
        ...
        '} catch(error) { console.error("[WEBVIEW] Fatal initialization error:", error); document.body.innerHTML = "<div style=\\"padding: 20px; color: red; font-family: monospace;\\"><h2>Webview Error</h2><p>" + error.message + "</p><pre>" + error.stack + "</pre></div>"; }' +
        '</script>' +
        '</body>' +
        '</html>';
